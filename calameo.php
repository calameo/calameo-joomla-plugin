<?php
/*
    Plugin Name: Content Calameo
    Description: Embed Calameo publications & miniCalameo inside a post
    Version: 3.0.0
    Author: Calameo
*/

/*  Copyright 2012 Calameo  (email : contact@calameo.com)
...
class plgContentCalameo extends JPlugin
{
	function onContentPrepare($context, $article, $params, $limitstart)
	{
		...
		// Joomla 2.5
		// Joomla 3.0
		$this->Calameo_parser($article, $params, $limitstart);
	}
	...
	function Calameo_parser($row, $params, $page = 0)
	{
		$regex = '/\[calameo (.*?)\]/i';
		preg_match_all($regex, $row->text, $matches);
		for ($x = 0; $x < count($matches[0]); $x++) {
			$replace = $this->calameo_render($matches[0][$x]);
